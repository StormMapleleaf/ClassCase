...
try{
    ...
    $stmt = $pdo->prepare($sql); //执行sql的预处理(为了防sql注入（安全）)

    //定义添加数据
    $data = array(
                array("name"=>"pp09","sex"=>"m","age"=>22,"classid"=>"lamp149"),
                array("name"=>"pp10","sex"=>"w","age"=>20,"classid"=>"lamp150"),
                array("name"=>"pp06","sex"=>"m","age"=>22,"classid"=>"lamp149"),
            );
    //遍历数据并实现批量添加
    $m = 0;
    foreach($data as $v){
